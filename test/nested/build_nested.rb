...
FileUtils.cd "test/nested" do
  ...
  puts %x{valac -C --library nested nested.vala --basedir ./ --vapidir=./../../vapi/ --pkg=Ruby}
  ...
  puts "linking nested..."
  puts %x{gcc --shared -fPIC -o nested.so $(pkg-config --cflags --libs gobject-2.0) nested.c -I/usr/local/lib/ruby/1.8/i686-linux}
  
end

puts "running VALAR..."
puts %x{ruby bin/valar test/nested/nested.vapi}
puts 
FileUtils.cd "test/nested" do
